...
namespace EchoBot1.Dialogs
{
    public class UserProfileDialog : ComponentDialog
    {
        ...
        private static async Task<DialogTurnResult> TicketStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            return await stepContext.PromptAsync(nameof(ChoicePrompt),
                new PromptOptions
                {
                    Prompt = MessageFactory.Text("Please choose your ticket."),
                    Choices = ChoiceFactory.ToChoices(new List<string> { "T1", "T2", "T3" }),
                }, cancellationToken);
        }

        private static async Task<DialogTurnResult> NameStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            stepContext.Values["ticket"] = ((FoundChoice)stepContext.Result).Value;
        ...
        private async Task<DialogTurnResult> SummaryStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            if ((bool)stepContext.Result)
            {
                ...
                userProfile.Ticket = (string)stepContext.Values["ticket"];

                var msg = $"I have your mode of transport as {userProfile.Transport} and your name as {userProfile.Name}";
                msg = msg + $" and your ticket is {userProfile.Ticket}";
